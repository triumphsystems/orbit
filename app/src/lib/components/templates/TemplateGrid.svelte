...
{#if loading && !hasTemplates}
	<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
		{#each Array(3) as _}
			<div class="bg-surface-900 border border-white/8 rounded-2xl p-4 animate-pulse space-y-3 h-44">
...
{:else if !hasTemplates}
	<div class="flex flex-col items-center justify-center py-20 gap-4 text-center">
		<div class="w-16 h-16 rounded-2xl bg-surface-900 border border-white/10 flex items-center justify-center">
		...
		<div>
			...
			<p class="text-xs font-mono text-slate-500 mt-1">Create your first PDF report template to get started.</p>
		</div>
		<button type="button" onclick={onNew} class="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs font-semibold font-sans hover:from-cyan-400 hover:to-blue-500 transition-all">
			Create Template
		</button>
	</div>
...
{:else}
	<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
		{#each templates as template (template.id)}
			<TemplateCard {template} onEdit={onEdit} onDelete={onDelete} onPreview={onPreview} />
		{/each}
	</div>
{/if}
